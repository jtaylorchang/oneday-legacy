    ...
    while ($row = $result->fetch_assoc()) {
        if ($password == $row['password']) {
            if ($row['verified'] == "1" || $row['verified'] == 1) {
                $b = mysqli_query($connect, "UPDATE users SET suspended='1' WHERE username LIKE '" . $target . "'");
                $r = mysqli_query($connect, "UPDATE users SET privacy='0' WHERE username LIKE '" . $target . "'");
