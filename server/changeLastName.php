...
$newLastName = trim(clean(str_replace('"', "", str_replace("'", "", $_GET["newlastname"]))));
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        if ($password == $row['password']) {
            if (strlen($newLastName) >= 1 && strlen($newLastName) <= 20) {
                $result2 = mysqli_query($connect, "UPDATE users SET lastName='" . $newLastName . "' WHERE username LIKE '" . $username . "'");
